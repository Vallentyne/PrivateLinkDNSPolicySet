# Build policy set with ONLY simple policies (no groupId complications)

Write-Host "`n=== Building Simplified Azure Private DNS Policy Set ===" -ForegroundColor Cyan
...
$policySet = @{
    properties = @{
        ...
        description = "Comprehensive policy set that deploys private DNS zones for $($policyDefs.Count) Azure PaaS services that support private endpoints. Includes Storage (7 types), AI/ML, IoT, Data Services, and Networking services."
        metadata = @{
            version = "2.0.0"
            category = "Network"
        }
...
$policySet | ConvertTo-Json -Depth 100 | Set-Content ".\simplifiedPolicySet.json"
Write-Host "`n✓ Created simplifiedPolicySet.json" -ForegroundColor Green

# Extract for PowerShell cmdlet
$policies | ConvertTo-Json -Depth 100 | Set-Content ".\simplifiedPolicies.json"
Write-Host "✓ Created simplifiedPolicies.json ($($policies.Count) policies)" -ForegroundColor Green

$parameters | ConvertTo-Json -Depth 100 | Set-Content ".\simplifiedParameters.json"
Write-Host "✓ Created simplifiedParameters.json ($($parameters.Count) parameters)" -ForegroundColor Green
