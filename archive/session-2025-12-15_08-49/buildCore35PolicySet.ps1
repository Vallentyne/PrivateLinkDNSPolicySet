# Build policy set with ONLY the 35 core services that are known to work

Write-Host "`n=== Building Core 35 Azure Private DNS Policy Set ===" -ForegroundColor Cyan

# Only the 35 validated and working policies
$policyDefs = @(
    ...
    @{id="a63cc0bd-cda4-4178-b705-37dc439d3e0f"; ref="DINE-Private-DNS-Azure-Cosmos-SQL"; param="azureCosmosSQLPrivateDnsZoneId"; display="Cosmos DB (SQL API)"; groupId="Sql"},
...
foreach ($pol in $policyDefs) {
    ...
    # Handle Cosmos DB group ID parameter
    if ($pol.groupId) {
        $policyRef.parameters["privateEndpointGroupId"] = @{
            value = $pol.groupId
        }
    }
...
$policySet = @{
    properties = @{
        ...
        description = "Comprehensive policy set that deploys private DNS zones for $($policyDefs.Count) Azure PaaS services that support private endpoints. Includes Storage (7 types), AI/ML, IoT, Data, Media, and Networking services."
        metadata = @{
            version = "2.0.0"
            category = "Network"
        }
...
$policySet | ConvertTo-Json -Depth 100 | Set-Content ".\comprehensivePolicySetFinal.json"
Write-Host "`n✓ Created comprehensivePolicySetFinal.json" -ForegroundColor Green

# Extract for PowerShell cmdlet
$policies | ConvertTo-Json -Depth 100 | Set-Content ".\comprehensivePoliciesFinal.json"
Write-Host "✓ Created comprehensivePoliciesFinal.json ($($policies.Count) policies)" -ForegroundColor Green

$parameters | ConvertTo-Json -Depth 100 | Set-Content ".\comprehensiveParametersFinal.json"
Write-Host "✓ Created comprehensiveParametersFinal.json ($($parameters.Count) parameters)" -ForegroundColor Green
